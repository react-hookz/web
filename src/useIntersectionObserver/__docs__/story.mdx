...
Tracks intersection of a target element with an ancestor element or with a top-level document's
viewport.
...
- **options** - Like `IntersectionObserver` options but `root` can also be react reference.
  - **root** _`RefObject<Element | Document> | Element | Document | null`_ _(default: `document`)_ -
    An `Element` or `Document` object (or its react reference) which is an ancestor of the intended
    target, whose bounding rectangle will be considered the viewport. Any part of the target not
    visible in the visible area of the root is not considered visible.
