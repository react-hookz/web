...
describe('useFavicon', () => {
  ...
  it('should set the elements href to the provided string', () => {
    renderHook(() => useFavicon('https://github.com/react-hookz/web'));
    const favicon = document.querySelector("link[rel*='icon']") as HTMLLinkElement;

    expect(favicon.href).toBe('https://github.com/react-hookz/web');
  });

  it('should update an existing favicon', () => {
    const hook = renderHook((props) => useFavicon(props), {
      initialProps: 'https://github.com/react-hookz/web',
    });
    const favicon = document.querySelector("link[rel*='icon']") as HTMLLinkElement;
